...
qqline(SARIMA2$residuals, col="red", lwd=2)
#Prueba Anderson-Darling
...
Por la simplicidad del modelo (Al tener solo 2 coeficientes), ser el que poseeel menor 
porcentaje de coeficientes no significativos y  además, pasar todos los tests (En 
normalidad pasó al menos Anderson-Darling) y tener los mejores índices, elegimos 
el ajustado por auto.arima

$$\therefore \textbf{Elegimos el } SARIMA(1,1,0)\times(1,1,0)_{\left[4\right]}$$ 
¿Ajustará mejor si le quitamos el coeficiente no significativo?

```{r}
modelo_automatico2<-arima(Serie_sq,order=c(0,1,0),seasonal=list(order=c(0,1,1),period=4))
modelo_automatico2
```
...
qqline(modelo_automatico2$residuals, col="red", lwd=2)
#Prueba Anderson-Darling
...
¡Pasa todos los tests y al parecer ajusta bien!

Solo queda ver los indicadores:

```{r}
AIC<-c(SARIMA$aic,ARIMA$aic,modelo_automatico$aic, SARIMA2$aic, modelo_automatico$aic)
BIC<-c(BIC(SARIMA),BIC(ARIMA),BIC(modelo_automatico), BIC(SARIMA2), BIC(modelo_automatico))
loglik<-c(SARIMA$loglik,ARIMA$loglik,modelo_automatico$loglik, SARIMA2$loglik, modelo_automatico$loglik)
...
comparar_4=cbind("SARIMA2",SARIMA2$aic,BIC(SARIMA2), mean(SARIMA2$residuals),
                 mean(abs(SARIMA2$residuals)),sqrt(mean((SARIMA2$residuals)^2)),
                 length(SARIMA2$coef))
comparar_5=cbind("SARIMA AUTOMÁTICO2",modelo_automatico2$aic,BIC(modelo_automatico2), mean(modelo_automatico2$residuals),
