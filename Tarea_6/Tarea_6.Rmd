...
```
¡ Tiene un p-value muy pequeño! Ello quiere decir que el coeficiente es 
significativo, y concuerda con lo obtenido en la tarea 5, donde vimos que 
para cada una de las etapas, el test de Log-Rank y de Wilcoxon generalizada
nos dice que son funciones de supervivencia distintos, por ende es natural que
haciendo un modelo de Cox, se tenga un parámetro significativo
## Modelo para edad
```{r}
age_model1<-coxph(Surv_general~age)
age_model1
```
...
```{r}
diagyr_model1<-coxph(Surv_general~diagyr)
diagyr_model1
```
...
modelo_conjunto
```
¡Conjuntamente, son significativos! El p-value es menor a 0.05
...
es menor que en el primer modelo conjunto obtenido en la línea anterior.
