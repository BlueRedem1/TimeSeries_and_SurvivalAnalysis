...
```{r}
Riesgo.1<-function(x,a=10,b=20){
  return(a+b*x)
}
Riesgo.2<-function(x,a=10,b=-20){
  return(a+b*x)
}
...
p <- ggplot() + xlim(0,100)
p + geom_function(fun=Riesgo.1, mapping = aes(color="b=20")
                  ) +
  geom_function(fun=Riesgo.2, mapping = aes(color="b=-20")
  ) +
  ...
  scale_color_manual(name = "Funciones de riesgo",
                     ...
                     labels = c("a=10,b=-20", "a=10,b=20", "a=10,b=5"))+
  labs(y= "h(t)", x = "t")+
...
```{r}
p <- ggplot() + xlim(0,2)
Riesgo_Acumulado.1<-function(x,a=10,b=20){
  return(a*x+(b*x^2)/2)
}
Riesgo_Acumulado.2<-function(x,a=10,b=-20){
  return(a*x+(b*x^2)/2)
}
Riesgo_Acumulado.3<-function(x,a=10,b=5){
  return(a*x+(b*x^2)/2)
}
p + geom_function(fun=Riesgo_Acumulado.1, mapping = aes(color="b=20")
                  ) +
  geom_function(fun=Riesgo_Acumulado.2, mapping = aes(color="b=-20")
  ) +
...
```{r}
p <- ggplot() + xlim(0,1)
Sobrevivencia.1<-function(x,a=10,b=20){
  return(exp(-(a*x+(b*x^2)/2)))
}
Sobrevivencia.2<-function(x,a=10,b=-20){
  return(exp(-(a*x+(b*x^2)/2)))
}
Sobrevivencia.3<-function(x,a=10,b=5){
  return(exp(-(a*x+(b*x^2)/2)))
}
p + geom_function(fun=Sobrevivencia.1, mapping = aes(color="b=20")
                  ) +
  geom_function(fun=Sobrevivencia.2, mapping = aes(color="b=-20")
  ) +
  ...
  theme(plot.title = element_text(hjust = 0.5))
```
Aquí viene un punto de conflicto que tuvimos: Y es que creemos que b debe ser 
un parámetro tal que $b\in\mathbb{R}^{+}$, ya que de lo contrario, notamos este
comportamiento no deseado en la función de sobrevivencia.
```{r}
Distribucion.1<-function(x,a=10,b=20){
  return(1-exp(-(a*x+(b*x^2)/2)))
}
Distribucion.2<-function(x,a=10,b=-20){
  return(1-exp(-(a*x+(b*x^2)/2)))
}
Distribucion.3<-function(x,a=10,b=5){
  return(1-exp(-(a*x+(b*x^2)/2)))
}
p + geom_function(fun=Distribucion.1, mapping = aes(color="b=20")
                  ) +
  geom_function(fun=Distribucion.2, mapping = aes(color="b=-20")
  ) +
  ...
  theme(plot.title = element_text(hjust = 0.5))
```
Lo mismo que en la gráfica anterior: b debría ser positivo, para evitar este comportamiento.
Vemos que mientras b sea más grande, más rápido converge a 1 
```{r}
p<- ggplot() +xlim(0,1)
Densidad.1<-function(x,a=10,b=20){
  return((a+b*x)*exp(-(a*x+(b*x^2)/2)))
}
Densidad.2<-function(x,a=10,b=-20){
  return((a+b*x)*exp(-(a*x+(b*x^2)/2)))
}
Densidad.3<-function(x,a=10,b=5){
  return((a+b*x)*exp(-(a*x+(b*x^2)/2)))
}
p + geom_function(fun=Densidad.1, mapping = aes(color="b=20")
                  ) +
  geom_function(fun=Densidad.2, mapping = aes(color="b=-20")
  ) +
  ...
  theme(plot.title = element_text(hjust = 0.5))
```
Comienza en $a$, y después decrece hasta alcanzar 0, pero si b es negativo, entonces
alcanza valores negativos, por eso también creemos que $b$ debería ser positivo.
...
Cuando $\gamma \leq 1$ es monótona decreciente;\\
Cuando $\gamma > 1$; es "parcialmente" ya que, en clase vimos que: 
\begin{center}
Cuando t es pequeña, comienza muy cerca de cero y va decreciendo hasta alcanzar un máximo, para después decrecer
\end{center}

La Gompertz cuando $\varphi > 1$; modela riesgos que 
\begin{align*}
S(t) &= e^{- \left(\frac{\lambda}{ln(\varphi)} \left(\varphi^{t} - 1\right)\right)}\\
h(t) &= \lambda \varphi^{t}
\end{align*}
