...
plot(Serie_ln)
# Con estp, ya tenemos nuestras series
...
legend("bottomright", col = c("darkblue","green"), lty = 1, lwd = 2, bty = "n",
       legend = c("Serie Homoscedastica","Yt=T+C+A"), cex = 1)



```
Veamos que ya la podemos descomponer. 
...
adf.test(wt)
```
Hacemos la diferencia con lag igual a 2, ya que en el inciso b) el periodo era de 2.
