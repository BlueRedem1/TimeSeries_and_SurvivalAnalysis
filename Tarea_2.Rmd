...
$$ 
\begin{aligned} \gamma(1) &= -0.4Cov(Z_{t}, Z_{t+1-1}) + .48 Cov(Z_{t-1}, Z_{t+1-2})\\
...
$$ 
\begin{aligned} \gamma(-1) &= -0.4Cov(Z_{t-1}, Z_{t-1}) + .48 Cov(Z_{t-2}, Z_{t-1-1})\\
...
$$ 
\begin{aligned} \gamma(2) &= -1.2Cov(Z_{t}, Z_{t+2-2})\\
                          &= -1.2Cov(Z_{t}, Z_{t})\\ 
                          &= -1.2Var(Z_{t})\\
                          &= -1.2\\
...
$$ 
\begin{aligned} \gamma(-2) &= -1.2Cov(Z_{t-2}, Z_{t-2})\\
...
Aplicamos:
```{r}
coefs_cor_ej2<-c(1,-8/35,-461/700)
phi_ej2<-c(-0.4,-0.75)
coefs_cor_ej2<-coefs_acf(phi_ej2,coefs_cor_ej2,10)
```

Entonces:
```{r}
Var_Xt<-1/(1-sum(phi_ej2*(coefs_cor_ej2[2:3])))
Var_Xt
```
Ahora dividimos entre la varianza para sacar los coeficientes del acf:
```{r}
acf_coefs_ej2<-coefs_cor_ej2/Var_Xt
